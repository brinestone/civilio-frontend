...
export class ElectronConfigService implements ConfigService {
	...
	async testDb(input: TestDbConnectionRequest) {
		return await sendRpcMessageAsync("db:test", input).then(
			TestDbConnectionResponseSchema.parse,
		);
	}

	async setLocale(locale: Locale) {
		return await sendRpcMessageAsync("locale:update", { locale }).then(
			AppConfigSchema.parse,
		);
	}

	async setTheme(theme: ThemeMode) {
		return await sendRpcMessageAsync("theme:update", { theme }).then(
			AppConfigSchema.parse,
		);
	}
