...
export const allSectionErrors = createSelector(
	[formSlices.activeSections],
	(sections) => {
...
export const undoAvailable = createSelector(
	[formSlices.undoStack],
	(r) => r.length > 0,
);
export const redoAvailable = createSelector(
	[formSlices.redoStack],
	(r) => r.length > 0,
);

export const changesPending = createSelector(
	[undoAvailable, redoAvailable],
	(u, r) => {
		return u;
	},
);
export const totalErrorCount = createSelector(
	[formSlices.activeSections],
	(sections) => {
		return values(sections)
			.map(({ errors }) => keys(errors).length)
			.reduce((acc, curr) => curr + acc, 0);
	},
);
export const migrationNeeded = createSelector(
	[configSlices.migrationState],
	(m) => m?.needsMigration === true,
);
export const dbConfig = createSelector([configSlices.knownConnections], (c) => {
	return c.find((d) => d.inUse);
});
export const hasConfiguredConnection = createSelector(
	[configSlices.knownConnections],
	(c) => c.some((x) => x.inUse),
);
export const connections = configSlices.knownConnections;
export const preInit = configSlices.preInit;
export const hasValidValidationCode = createSelector(
	[formSlices.activeSections, formSlices.schemas],
	(sections, schemas) => {
		const validationCodeKeys = [
			[
				"csc.form.sections.extra",
				"csc.form.sections.extra.fields.validation_code",
			],
			["fosa.form.sections.extras", "fosa.form.fields.validation_code"],
			[
				"chefferie.form.sections.comments",
				"chefferie.form.sections.comments.fields.validation_code",
			],
