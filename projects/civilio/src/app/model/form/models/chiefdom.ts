...
const relevanceMap = {
	...
	centerHasEmployees: {
		dependencies: [
			"chefferie.form.sections.personnel_status.general.fields.employee_count",
		],
		predicate: RelevancePredicateSchema.implement((deps) => {
			const count = Number(
				deps[
	...
	centerUsesOtherWaterSource: {
		...
		predicate: RelevancePredicateSchema.implement((deps) => {
			return (
				intersection(
					["6"],
					[deps["chefferie.form.sections.infra.fields.water_sources"]],
				).length > 0
			);
		}),
	} as RelevanceDefinition,
	centerUsesWater: {
		...
		predicate: RelevancePredicateSchema.implement((deps) => {
			return (
				deps["chefferie.form.sections.infra.fields.has_water_source"] === true
			);
		}),
	} as RelevanceDefinition,
		...
		},
		{
			...
			fields: [
				...
				},
				{
					...
					type: "table",
					identifierColumn:
						"chefferie.form.sections.personnel_status.employees.fields.index",
					columns: {
						index: {
							visible: false,
							editable: false,
