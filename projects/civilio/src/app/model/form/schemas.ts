...
export const RelevanceLogicExpressionInputSchema =
  RelevanceLogicExpression.extend({
...
export type RelevanceLogicExpressionInput = z.input<
  typeof RelevanceLogicExpressionInputSchema
>;
...
export const DateFieldTypesSchema = FieldTypeSchema.extract([
  "date",
  "multi-date",
  "date-range",
  "date-time",
...
export const SimpleDateFieldItemConfigSchema =
  BaseDateFieldItemConfigSchema.extend({
...
export const SelectFieldItemConfigSchema = BaseFieldItemConfigSchema.extend({
  ...
  itemSourceRef: z
    ...
    .default(
      isDevMode()
        ? "refs:dataset::aecbbee7b14a7b0324721bbcbb4359aaa796858fa92cbe01e32adbad35478f7b"
        : null,
    ),
  defaultValue: z.any().nullish().default(null),
...
const FieldValueBaseSchema = z.union([
  z.string(),
  z.number(),
  z.date(),
  z.boolean(),
...
const FieldValueSchema = z.union([
  FieldValueBaseSchema,
  FieldValueBaseSchema.array(),
]);
...
export const ValueProviderFnSchema = z.function({
  ...
  output: z.record(
    FieldKeySchema,
    z.union([FieldValueSchema, FieldValueSchema.array()]).nullable(),
  ),
});
export const RelevancePredicateSchema = z.function({
  input: [
    z.record(
      FieldKeySchema,
      z.union([ParsedValueSchema, ParsedValueSchema.array()]),
    ),
  ],
  output: z.boolean(),
});
...
const TabularFieldDefinitionSchema = BaseFieldDefinitionSchema.omit({
  span: true,
  required: true,
  default: true,
}).extend({
...
export const FormGroupSchema = GroupBaseSchema.extend({
  ...
  columns: z.union([z.int(), z.string().array()]).optional(),
});
