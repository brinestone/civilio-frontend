...
@Component({
	...
	imports: [DecimalPipe, NgClass, HlmButton, NgIcon],
...
export class GeoPointPicker
	implements AfterViewInit, FormValueControl<GeoPoint | null | undefined>
{
	...
	public readonly clearable = input<boolean, BooleanInput>(true, {
		transform: booleanAttribute,
	});
	...
	protected readonly lat = linkedSignal(
		() => this.value()?.lat ?? 3.8614800698189145,
	);
	protected readonly long = linkedSignal(
		() => this.value()?.long ?? 11.520851415955367,
	);
	protected readonly resolvedCoords = computed(() =>
