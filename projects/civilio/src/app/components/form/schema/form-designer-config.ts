...
import { NewFormItemImage } from "@civilio/sdk/models";
import {
	BooleanFieldConfig,
	FieldItemConfig,
	FormItemDefinition,
...
export function defineFormDesignerFormSchema() {
	return (paths: SchemaPathTree<Strict<FormVersionDefinition>>) => {
		hidden(paths.id, () => true);
		...
		applyEach(paths.items, (itemPaths) => {
			hidden(
				itemPaths.relevance,
				({ valueOf }) =>
					!["field", "group", "image"].includes(valueOf(itemPaths.type)),
			);
			disabled(
				itemPaths.relevance.logic,
				({ valueOf }) => valueOf(itemPaths.relevance.enabled) !== true,
			);
			disabled(
				itemPaths.relevance.operator,
				({ valueOf }) => valueOf(itemPaths.relevance.enabled) !== true,
			);
			disabled(itemPaths.relevance, ({ valueOf }) => {
				const flatFields = valueOf(paths.items).flatMap((i) =>
					i.type == "group" && !i.config.repeatable
						? i.config.fields
						: i.type == "field"
							? [i]
							: [],
				...
				const isFieldItem = valueOf(itemPaths.type) == "field";
				if (isFieldItem && fieldCount < 2) {
					return "There must be at least 1 other question in the form";
				} else if (!isFieldItem && fieldCount < 1) {
					return "There must be at least 1 question in the form";
				}
				return false;
			});
			hidden(
				itemPaths.relevance.operator,
				({ valueOf }) => valueOf(itemPaths.relevance.logic).length < 2,
			);
			applyEach(itemPaths.relevance.logic, (conditionPaths) => {
				hidden(
					conditionPaths.operator,
					({ valueOf }) => valueOf(conditionPaths.expressions).length < 2,
				);
			});
			applyEach(itemPaths.tags, (tagPaths) => {
				...
				pattern(tagPaths.key, /^[a-zA-Z0-9_-]*$/, {
					message:
						"Invalid format. Key must contain only letters, numbers, underscores or hyphens",
				});
			});
		});
	};
