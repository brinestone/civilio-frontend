...
@Component({
	...
	imports: [HlmButton, NgIcon, HlmToggle],
...
export class FormItemActions {
	...
	protected readonly actions = signal<FormItemAction[]>([
		{
			toggle: (state: "on" | "off") => this.toggleSettings.emit(state == "on"),
			icon: "lucideSettings",
			label: "Toggle Settings",
			type: "toggle",
		},
		{
			toggle: (state: "on" | "off") => this.togglePreview.emit(state == "on"),
			icon: "lucideEye",
			label: "Preview",
			type: "toggle",
		},
		{
			handler: () => this.duplicate.emit(),
			icon: "lucideCopy",
