...
export type FormSchemaContext = {
	...
	allFields: Signal<
		Record<string, FieldTree<Strict<FormItemField | NewFormItemField>>>
	>;
...
const FormDesignerContextToken = new InjectionToken<FormSchemaContext>(
	"forms.ctx",
);
const FormItemDesignerContextToken = new InjectionToken<
	FormItemDesignerContext<FormItem>
>("forms.item.ctx");
...
export function createFormItemDesignerContextInjector<T extends FormItem>(
	context: FormItemDesignerContext<T>,
	...
	return Injector.create({
		...
		providers: [
			{
				provide: FormItemDesignerContextToken,
				multi: false,
				useValue: context,
			},
		],
...
export function createFormSchemaContextInjector(
