...
export class FormItemRelevanceConfig {
	...
	private readonly booleanExpressionValueTemplate = viewChild.required<
		TemplateRef<any>
	>("booleanExpressionValueTemplate");
	private readonly simpleDateExpressionValueTemplate = viewChild.required<
		TemplateRef<any>
	>("simpleDateExpressionValueTemplate");
	private readonly multiDateExpressionValueTemplate = viewChild.required<
		TemplateRef<any>
	>("multiDateExpressionValueTemplate");
	private readonly rangeDateExpressionValueTemplate = viewChild.required<
		TemplateRef<any>
	>("rangeDateExpressionValueTemplate");
	private readonly selectionExpressionValueTemplate = viewChild.required<
		TemplateRef<any>
	>("selectionExpressionValueTemplate");
	private readonly numberExpressionValueTemplate = viewChild.required<
		TemplateRef<any>
	>("numberExpressionValueTemplate");
	private readonly textExpressionValueTemplate = viewChild.required<
		TemplateRef<any>
	>("textExpressionValueTemplate");
	...
	protected readonly patternPresets = [
		...
		},
		{
			...
			label: "Phone",
			icon: "lucidePhone",
			regex: `^(\\+?237|\\(\\+?237\\))?6([5679]|[2])\\d{7}$`,
		},
		{
			name: "url",
			label: "URL",
			icon: "lucideGlobe",
			regex: `^((https?|ftps?):\\/\\/)?((\\d{1,3}\\.){3}\\d{1,3}|(www\\.)?[-a-zA-Z0-9@:%._\\+~#=]{1,256}\\.[a-zA-Z0-9()]{1,6})\\b([-a-zA-Z0-9()@:%_\\+.~#?&//=]*)$`,
		},
	];
	protected readonly expressionValueTemplates = {
		...
		keyof typeof fieldTypeExpressionOperatorsMap,
		Signal<TemplateRef<any>>
	>;
	protected readonly fieldMap = this.formContext.allFields;
	protected readonly otherFieldsList = computed(() => {
		return values(this.formContext.allFields()).filter(
			(f) => f.path().value() !== untracked(this.path),
		);
	});
	protected readonly fieldTypeExpressionOperatorsMap =
	...
	protected onAddExpressionButtonClicked(conditionIndex: number) {
		this.relevance()
			.logic[conditionIndex].expressions()
			.value.update((expressions) =>
	...
	protected onRemoveExpressionButtonClicked(
