...
export class DateRangePicker
	implements FormValueControl<undefined | null | NumberRange>, OnInit, OnDestroy
{
	public readonly value = model<NumberRange | undefined | null>(undefined);
	public readonly userClass = input<ClassValue>("", { alias: "class" });
	public readonly buttonId = input<string>(`date-range-picker-${++nextId}`, {
		alias: "id",
	});
	...
	public readonly clearable = input<boolean, unknown>(false, {
		transform: booleanAttribute,
	});
	...
	constructor() {
		effect(() => {
			...
			if (diff > 0) return;
			this.value.update((v) =>
				produce(v ?? { start: null, end: null }, (draft) => {
					draft.start = min;
				}),
			);
		});
