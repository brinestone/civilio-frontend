...
import { Channel, ChannelArg, ChannelResponse } from "@civilio/shared";
import {
  Actions,
  ActionType,
  ofActionCompleted,
...
/**
 * Strips symbol properties from an object and returns the stripped copy.
 * @param o the object
 * @returns a clone of o with all symbols removed
 */
export function stripSymbols<T extends object>(o: T) {
  const symbols = Object.getOwnPropertySymbols(o);
  return omit(o, symbols);
}

export function debouncedAsyncValidator<T>(
  duration: number,
  logic: (
    value: T,
...
export function maskString(value: string, maskChar = ".", maskMaxLength = 3) {
  ...
  const start = Math.floor(0.333 * value.length);
  const end = Math.floor(0.8 * value.length);
