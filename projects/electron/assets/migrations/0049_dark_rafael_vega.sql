...
BEGIN
	...
	_parent_version := COALESCE(
		NULLIF(current_setting('parent_version', true), ''),
		old_data ->> '_version_',
	...
	_new_version := COALESCE(
		NULLIF(current_setting('session.new_version', true), ''),
		MD5(
