CREATE OR REPLACE FUNCTION revisions.func_log_submission_state(
	IN p_index INTEGER,
...
BEGIN
	...
	EXECUTE FORMAT('ALTER TABLE %I.data DISABLE TRIGGER USER', p_form);
	_main_data := _main_data - '_version_';
	_new_hash := MD5(_main_data::TEXT || '.' || COALESCE(_main_data ->> '_submitted_by', SESSION_USER)::TEXT || '.' ||
									 'INSERT');
	...
	EXECUTE FORMAT('ALTER TABLE %I.data ENABLE TRIGGER USER', p_form);

	INSERT INTO revisions.ledger(hash, form, timestamp, submission_index, notes)
	VALUES (_new_hash, p_form, _ts, p_index, 'Rev 1')
