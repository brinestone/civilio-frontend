	...
	IF TG_OP IN ('UPDATE', 'INSERT') THEN
		FOR k IN SELECT skeys(diff_store)
			LOOP
				...
				-- Check if the value changed to NULL
--                 IF (new_store -> k) IS NOT NULL THEN
--                     delta_json := delta_json || jsonb_build_object(k, to_jsonb(new_store -> k));
--                 ELSIF (new_store -> k) IS NULL AND (old_store -> k) IS NOT NULL THEN
--                     -- The value at k changed from a non-null value to a null value
