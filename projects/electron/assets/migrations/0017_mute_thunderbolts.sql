...
BEGIN
	...
	actor := COALESCE(
		...
		(new_data -> '_submitted_by')::TEXT,
		(new_data -> '_submission_submitted_by')::TEXT,
		SESSION_USER
					 );

	form_type := TG_TABLE_SCHEMA::civilio.form_types;
	_parent_version := COALESCE(new_data -> '_version_', old_data -> '_version_', new_data -> '_submission_version',
															old_data -> '_submission_version');
	_submission_index := COALESCE(new_data -> '_index', old_data -> '_index', new_data -> '_parent_index',
																old_data -> '_parent_index');
	...
	_now := NOW();
	_new_version :=
		md5(delta_json::TEXT || '.' || _now::TEXT || '.' || _submission_index::TEXT || '.' || tg_table_name::text ||
				'.' || actor || '.' || tg_op || '.' || _parent_version);
