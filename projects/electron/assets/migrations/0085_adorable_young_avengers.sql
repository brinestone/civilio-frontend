...
CREATE OR REPLACE PROCEDURE revisions.sync_version(
...
BEGIN
	_version := COALESCE(
		p_version,
		NULLIF(current_setting('session.working_version'), ''),
...
DECLARE
	...
	_is_syncing       BOOLEAN;
BEGIN
	_is_syncing = COALESCE(NULLIF(current_setting('session.syncing'), '') = 'true', FALSE);
	IF _is_syncing THEN
