...
DECLARE
	...
	_is_syncing       BOOLEAN;
BEGIN
	_is_syncing = COALESCE(NULLIF(current_setting('session.syncing', TRUE), '') = 'true', FALSE);
	IF _is_syncing THEN
		RAISE NOTICE 'Synchronization in process. Skipping auditing';
		...
		FOR k IN SELECT skeys(diff_store)
			LOOP
				CONTINUE WHEN k IN (
														...
														'_submitted_by', '_submission_submitted_by', '_validation_status'
					);
	...
	VALUES (_new_version,
					...
					_now,
					_submission_index,
					COALESCE(NULLIF(current_setting('session.notes', true), ''), 'N/A'))
	ON CONFLICT (hash, form, submission_index) DO NOTHING;
