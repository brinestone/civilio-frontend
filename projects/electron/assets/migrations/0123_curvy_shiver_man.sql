DROP VIEW IF EXISTS "civilio"."vw_facilities";--> statement-breakpoint
...
$$
	DECLARE
		...
		_schema   TEXT;
	BEGIN
		FOR _col_name, _schema IN SELECT c.column_name::TEXT, c.table_schema::TEXT
															FROM information_schema.columns c
															WHERE c.table_schema IN ('csc', 'chefferie', 'fosa')
																AND c.data_type = 'character varying'
																AND c.column_name = '_validation_status'
			LOOP
				EXECUTE FORMAT('ALTER TABLE %I.data ALTER COLUMN %I SET DATA TYPE TEXT', _schema, _col_name);
			end loop;
