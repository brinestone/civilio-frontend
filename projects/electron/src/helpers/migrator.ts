...
export class MigrationRunner<TDb extends PgDatabase<any, any>> {
	...
	async checkMigrations(db: TDb): Promise<{
		...
		const lastAppliedName =
			appliedData.rows.length > 0 ? (appliedData.rows[0].name as string) : null;
	...
	async runMigrations(db: TDb) {
		...
		this.logger.log(
			`Applying ${status.pending.length} new migrations (starting after ${status.lastAppliedName ?? "beginning"})...`,
		);
					...
					for (const statement of statements) {
						try {
							...
							if ("code" in e && IDEMPOTENT_ERROR_CODES.has(e.code)) {
								this.logger.warn(
									`Migration ${migration.name} skipped a statement (Object already exists).`,
								);
							} else {
								throw e; // Rollback
							}
