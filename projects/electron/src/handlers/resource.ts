...
export async function getLicences() {
	const filePath = app.isPackaged
		? join(app.getPath("assets"), "resources", "assets", "3rdpartylicenses.txt")
		: resolve(join(__dirname, "..", "assets", "3rdpartylicenses.txt"));
	...
	for (const entry of entries) {
		...
		const packageName = packageLine
			? packageLine.replace("Package:", "").trim()
			: "N/A";
		const licenseType = licenseTypeLine
			? licenseTypeLine.replace("License:", "").replace(/"/g, "").trim()
			: "N/A";
		const licenseTextLines = lines.filter(
			(line) =>
...
export async function getResourceUrl(fileName: string) {
	const filePath = app.isPackaged
		? join(app.getPath("assets"), "resources", "assets", fileName)
		: resolve(join(__dirname, "..", "assets", fileName));
	if (!existsSync(filePath)) {
		return null;
	}
