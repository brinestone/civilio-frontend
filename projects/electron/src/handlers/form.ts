...
export async function processSubmissionDataUpdate({
	...
	return await db.transaction(async tx => {
		...
		let index = Number(submissionIndex);
		...
		if (submissionIndex !== undefined && deltas.some(d => {
			...
			return;
		} else if (submissionIndex === undefined || isNaN(index)) {
			...
			index = result.rows[0]._index as number;
		}
		const tableGroupedMappings = groupBy(allMappings, 'dbTable');
		for (const [table, mappings] of entries(tableGroupedMappings)) {
			const tableDeltas = intersectionBy(deltas, mappings, 'field');
			console.log(tableDeltas);
			if (table == 'data') {
				...
				if (additions && additions.length > 0) {
					const uniqueMappings = additions.flatMap(entries).reduce((acc, [k]) => {
					...
					for (const row of additions) {
						...
						const cols = [...sequences[form][table].map(({ column }) => column), ...kvm.keys()].map(c => sql.identifier(c));
						const values = [...sequences[form][table].map(({ sequence }) => sql`nextval('${ sequence }')`), ...[...kvm.values()].map(([v, t]) => sql`${ v || null }::${ sql.raw(t) }`)];
						await tx.execute(sql.raw(`
							INSERT INTO ${ form }.${ table } (${ sql.join(cols, sql`, `) })
				...
				if (deletions && deletions.length > 0) {
					let identifierMapping = mappings.find(m => m.field == deletions[0].identifierKey);
					if (!identifierMapping) {
						throw new Error('Could not identify identifier mapping. Cannot proceed');
					}
					...
					await tx.execute(sql`
						...
						WHERE ${ and(
							eq(sql.identifier('_parent_index'), index),
