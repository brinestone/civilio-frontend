...
> = {
	...
	csc: {
		...
		data_archives: [{ column: "_index", sequence: cscIndexSeqInCivilio }],
		data_personnel: [
			{ column: "_index", sequence: cscPersonnelIndexSeqInCivilio },
		],
		data_pieces: [{ column: "_index", sequence: cscPiecesIndexSeqInCivilio }],
		data_statistiques: [
			{ column: "_index", sequence: cscStatisticsIndexSeqInCivilio },
		],
		data_villages: [{ column: "_index", sequence: cscVillagesSeqInCivilio }],
	},
	chefferie: {
		data: [
...
export async function getSubmissionInfo({
	...
	const [result] = await db
		...
		.from(vwFacilities)
		.where(
			and(eq(vwFacilities.index, index as number), eq(vwFacilities.type, form)),
		)
		.limit(1);
...
export async function processSubmissionDataUpdate(
	...
	return await db.transaction(async (tx) => {
		let newVersion =
			customVersion ||
			hashThese([JSON.stringify(deltas), Date.now()].join("|"));
		logger.debug("new version = ", newVersion);
		...
		if (isNew) {
			...
			const tableDeltas = deltas.filter((delta) => {
				return dataTableMappings.some(
					(mapping) => mapping.field == delta.field,
				);
			});
			const dataEntries = tableDeltas.reduce(
				(acc, curr) => {
					const mapping = dataTableMappings.find((m) => m.field == curr.field)!;
					acc[mapping.dbColumn] =
						sql`${curr.value || null}::${sql.raw(mapping.dbColumnType)}`;
					return acc;
				},
			...
			const columns = [
				...requiredCols.map((c) => sql.identifier(c.column)),
				...keys(dataEntries).map((k) => sql.identifier(k)),
			];
			const dataValues = [
				...requiredCols.map((c) =>
					sql.raw(`nextval('${c.sequence.schema}.${c.sequence.seqName}')`),
				),
				...values(dataEntries),
			];

			const result = await tx.execute(sql`
				INSERT INTO ${sql.identifier(form)}."data" ("_submission_time", ${sql.join(
					columns,
					sql`,
																			`,
				)})
			...
			if (result.rows.length == 0) {
				throw new Error(
					"An unexpected error occurred. Please try again later or contact your administrator",
				);
			}
		...
		for (const [table, mappings] of entries(tableGroupedMappings)) {
			...
			const tableDeltas = deltas.filter((delta) => {
				const fieldToCheck =
					delta.op === "add" && delta.identifierKey
						? delta.identifierKey
						: delta.field;
				return mappings.some((mapping) => mapping.field === fieldToCheck);
			});
			logger.debug(
				`Processing ${tableDeltas.length} updates for table: ${table}`,
			);
			...
			if (table == "data" && tableDeltas.length > 0) {
				...
				const updates = [
					...kvm
						...
						.map(
							([col, [v, t]]) =>
								sql`${sql.identifier(col)} = ${v || null}::${sql.raw(t)}`,
						),
				];
				await tx.execute(sql`
				...
				if (additions && additions.length > 0) {
					...
					const uniqueMappings = additions
						.flatMap((a) => keys(a.value))
						.reduce(
							(acc, k) => {
					...
					const identifierMapping = mappings.find(
						(m) => m.field == additions[0].identifierKey,
					);
					if (!identifierMapping) {
						throw new Error(
							"Could not identify identifier mapping. Cannot proceed",
						);
					}
					for (const row of additions) {
						...
						const cols = [
							...sequences[form][table].map(({ column }) => column),
							...kvm.keys(),
							"_parent_index",
						].map((c) => sql.identifier(c));
						const values = [
							...sequences[form][table].map(
								({ sequence }) =>
									sql`nextval('${sql.join([sql.raw(sequence.schema), sql.raw(sequence.seqName)], sql`.`)}')`,
							),
							...[...kvm.values()].map(
								([v, t]) => sql`${v || null}::${sql.raw(t)}`,
							),
							_submission_index,
						];
						await tx.execute(sql`
				...
				if (deletions && deletions.length > 0) {
					...
					let identifierMapping = mappings.find(
						(m) => m.field == deletions[0].field,
					);
					if (!identifierMapping) {
						throw new Error(
							"Could not identify identifier mapping. Cannot proceed",
...
export async function initializeSubmissionVersioning({
	...
	//language=PostgreSQL
	const queryResult =
		await db.execute(sql`SELECT revisions.func_log_submission_state(
																										${index},
																										${form}) AS version`);
	return InitializeSubmissionVersionResponseSchema.parse(
		queryResult.rows[0]?.version ?? null,
	);
}
...
export async function findCurrentSubmissionVersion({
	...
	return FindSubmissionCurrentVersionResponseSchema.parse(
		queryResult.rows[0] ?? null,
	);
}
...
export async function findSubmissionRef({
	...
	const [result] = await db
		...
		.where(
			and(
				eq(vwFormSubmissions.index, Number(index)),
				eq(vwFormSubmissions.form, form),
			),
		);
...
export async function findFormSubmissions(
	...
	try {
		...
		const filter = filterQuery
			? and(
					eq(vwFormSubmissions.form, form),
					or(...searchColumns.map((col) => like(sql`LOWER(${col}::TEXT)`, q))),
				)
			: eq(vwFormSubmissions.form, form);
