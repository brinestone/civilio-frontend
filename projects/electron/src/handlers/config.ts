...
export async function discoverServer() {
	const port = 5534;
	const clientKey =
		"g8eULU5uY6u1ZQSuGTUeDQABLgArt2yOTDKJ0AB1Y13GYCHt6Hkj7Q0qsUGWtRqS";
	...
	return await new Promise<DiscoverServerResponse>((resolve, reject) => {
		...
		client.once("message", (msg, rinfo) => {
			try {
				...
				logger.info(
					`Found server at ${serverData.baseUrl} via ${rinfo.address}`,
				);
		...
		client.bind(() => {
			...
			const message = Buffer.from(
				JSON.stringify({ c: `DISCOVER=${clientKey}` }),
			);
...
export function getBuildInfo() {
	const manifestPath = app.isPackaged
		? join(app.getPath("assets"), "resources", "assets", "build.json")
		: resolve(join(__dirname, "..", "assets", "build.json"));
