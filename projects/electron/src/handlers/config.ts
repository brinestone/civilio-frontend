...
export async function discoverServer() {
	...
	return new Promise<DiscoverServerResponse>((resolve, reject) => {
		const client = createSocket('udp4');
		client.on('message', (msg, rinfo) => {
			try {
				const serverData = JSON.parse(msg.toString());
				console.log(`Found server at ${serverData.baseUrl} via ${rinfo.address}`);
				resolve(DiscoverServerResponseSchema.parse(serverData));
			} catch (e) {
				logger.error(e);
				reject(e);
			} finally {
