    ...
    public void setupValueChangedListeners() {
        ...
        listChanged.addListener((ob, ov, nv) -> {
            tvPersonnel.getItems().stream()
                    .filter(CHEFFERIEPersonnelInfoViewModel::isSelected)
        ...
        field.valueProperty().addListener((ob, ov, nv) -> {
            final var vms = nv.stream()
                    .map(CHEFFERIEPersonnelInfoViewModel::new)
                    ...
                    .toList();
            tvPersonnel.getItems().setAll((CHEFFERIEPersonnelInfoViewModel) vms);
        });
        selectedItems.addListener((SetChangeListener<CHEFFERIEPersonnelInfoViewModel>) c -> {
    ...
    public void initializeParts() {
        ...
        tvPersonnel.getItems().setAll(
                Optional.ofNullable(field.getValue())
                        ...
                        .peek(vm -> vm.selectedProperty().addListener(CHEFFERIEPersonnelInfoControl.this::onItemSelectionStatusChanged))
                        .toList()
        );
