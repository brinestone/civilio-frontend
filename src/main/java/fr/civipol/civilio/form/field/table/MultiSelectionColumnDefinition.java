...
public class MultiSelectionColumnDefinition<V, R> extends ColumnDefinition<V, List<R>, MultiSelectionColumnDefinition<V, R>> {
    private static <V, R> Supplier<TableCell<V, List<R>>> defaultSupplier(ObservableList<R> options, StringConverter<R> stringConverter) {
        return () -> new CheckComboBoxTableCell<>(stringConverter, options);
    }
