...
async function translateValue(value, sourceLocale, targetLocales) {
  ...
  const cachedTranslations = await findTranslationsByDigest(
    digest,
    localesCopy,
  );
...
async function translateFile(filePath, sourceLocale, targetLocales) {
  ...
  const prefix = fileName.substring(
    0,
  ...
  try {
    ...
    console.log(
      `Translating file ${filePath} from ${sourceLocale} to ${targetLocales.join(", ")}`,
    );
    for (const targetLocale of targetLocales) {
      const targetFilePath = join(
        dirname(filePath),
        `${prefix}_${targetLocale}${suffix}`,
      );
      if (existsSync(targetFilePath)) {
        console.log(
          `Backing up existing translation file for target locale: ${targetLocale}`,
        );
        const backupFilePath = targetFilePath + "." + Date.now() + ".bak";
        await rename(targetFilePath, backupFilePath).then(() =>
          console.log(`Backup created at ${backupFilePath}`),
        );
        await clearFile(targetFilePath);
        console.log(
          `Cleared existing translation files for target locales: ${targetLocales.join(", ")}`,
        );
      }
    }
...
async function cacheTranslationsFor(
  key,
  srcLocale,
  srcValue,
  destLocale,
